...
class Akademik extends CI_Controller {
	...
	{
		$this->template->load('template_admin', 'akademik/jadwal_mengajar/list_jadwal_mengajar');
	}
	...
	public function form_jadkul($id = null)
	{
		if($id == null){
			...
			$data['action'] = 'akademik/create_jadkul';
			
			$this->template->load('template_admin', 'akademik/jadwal_mengajar/form_jadwalkuliah', $data);
		}
		else {
			$data = $this->m_akademik->get_jadkul_byid($id)->row_array();
			$data['dosen'] = $this->m_akademik->get_dosen()->result_array();
			$data['title'] = 'Edit Jadwal Mengajar';
			$data['action'] = 'akademik/update_jadkul';
	...
	public function hapus_jadkul($id)
	{
		...
		if($this->m_akademik->delete('tbl_info_jadwal_kuliah', array('id_jadwal_kuliah' => $id))){
			$this->session->set_flashdata('msg', 1);
			redirect('akademik/jadwal_kuliah');
		} 
	}
	...
	{
		$this->template->load('template_admin', 'akademik/Mata_kuliah/dashboard_matakuliah');
	}
	...
	public function form_matkul($id = null)
	{
		if($id == null){
			...
			$data['action'] = 'akademik/create_matkul';
			
			$this->template->load('template_admin', 'akademik/Mata_kuliah/form_matakuliah', $data);
		}
		else {
			
			$data = $this->m_akademik->get_matkul_byid($id)->row_array();
			$data['title'] = 'Edit Mata Kuliah';
			$data['action'] = 'akademik/update_matkul';
