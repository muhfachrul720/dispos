    ...
    <div class="card shadow mb-4 p-5" style="font-size:14px">
        <div class="header"> 
            <div class="row justify-content-between">
                <div class="col-6">
                    <h3 class="mb-3">List Ajuan Kenaikan Pangkat Jabatan Reguler (Terverifikasi)</h3>
                    <p class="mb-0">List Pengajuan Kenaikan Jabatan Yang telah disetujui </p>
                </div>
                <div class="col-6" style="text-align:right;">
                    <a href="<?= base_url()?>cetakexcel/print_excel_reguler_nonsk" class="btn btn-success">Export Pegawai Non SK</a>
                    <a href="<?= base_url()?>cetakexcel/print_excel_reguler_all" class="btn btn-success">Export Semua Pegawai</a>
                </div>
            </div>
        </div>
        <hr>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var t = $("#example").dataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: {"url": '<?= base_url()?>pegawai/json_mon_naikpangkat_reguler', "type": "POST", data : {'id' : <?= $this->session->userdata('id_pegawai')?>}},
                    columns: [
                        ...
                        {
                            "data" : 'report_pengajuan_reguler',
                            "render" : function(data, type, row){
                                if(data == null){
                                    return '<button type="button" class="btn btn-primary" data-book-id="'+row.id_ajuan_reguler+'" data-toggle="modal" data-target="#exampleModalCenter">Upload Surat</button>'
                                }
                                else {
                                    return '<a href="<?= base_url()?>upload/report_naikpangkat/reguler/'+data+'" class="btn btn-success py-2" style="font-size:12px !important" download>Download Surat</a>';
                                }
                            }
                        },
                        {
                            ...
                            "render" : function(data, type, row){
                                if(row.report_pengajuan_reguler != null){
                                    return '<a href="<?=base_url()?>pegawai/ubah_pangkat_golongan/'+data+'" style="color:white" class="btn btn-primary">Ubah Pangkat</a>';
                                }
                                else {
    ...
    <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <?= form_open_multipart('pegawai/upload_feedback_reguler');?>
