<?php if($id_kategori_jabatan_fung == 0 || $id_kat_jbt_struktur == 12 ) {?>
    <div class="container-fluid">
        <h2 style="text-align:center; margin-top:20vh">Hanya Pegawai Yang Menjabat Salah Satu Jabatan Fungsional Dan Non Dosen Yang Dapat Mengajukan Pengajuan Kenaikan Pangkat </h2>
    </div>
<?php } else {?>
        <?php if(notif_kenaikanpangkat($tmt_jab_fungsional) == false) {?>
            <div class="container-fluid">
                <h2 style="text-align:center; margin-top:20vh">Anda Harus Memenuhi Kriteria 2 Tahun masa Kerja dari Jabatan Fungsional Sebelumnya</h2>
            </div>
        <?php } else {?>
        <div class="container-fluid">
            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane show active fade" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="card border-top-0 mb-3 pt-3 px-3 pb-1">
                        <div class="card-body">
                            <h3 class="">Form Pengajuan Naik Pangkat Jabatan Fungsional </h3>
                            <p>Silahkan Mengupload syarat syarat yang diperlukan dalam mengurus Pensiunan Secara Lengkap dan benar</p>
                            <?php if(isset($aju)) {?> 
                                <div class="alert alert-info alert-sm" role="alert">
                                <small> Keterangan Koreksi : <?= $aju['keterangan_pengajuan_fungsional']?> </small>
                                </div>
                            <?php }?> 
                        </div>
                    </div>  
                    ...
                    <?php $checker = array(
                        ...
                        array('<b>Foto Kopi Sah</b> SK Jabatan Fungsional Tertentu', 'skjabfung'),
                        array('<b>Foto Kopi Sah</b> SK Pembebasan sementara dan pengangkatan kembali dalam JFT', 'skjft'),
                        array('<b>SKP, Capaian SKP, dan Penilaian Prestasi Kerja</b> (2 Tahun terakhir <b>sekurang-kurangnya bernilai baik</b>', 'skp'),
                        array('Penilaian Angka Kredit <b>Asli</b> (PAK)', 'pak'),
                    );?>

                    <?php if(isset($aju)){ 
                        echo form_hidden('time', $aju['waktu_pengajuan_fungsional']);
                        echo form_hidden('id', $aju['id_ajuan_fungsional']);
                        ...
                        <script type="text/javascript">
                            $(document).ready(function() {
                                ...
                                var t = $("#example").dataTable({
                                    ...
                                    processing: true,
                                    serverSide: true,
                                    ajax: {"url": '<?= base_url()?>dashboard_p/json_naikpangkat_fungsional', "type": "POST", data : {'id' : <?= $this->session->userdata('id_pegawai')?>}},
                                    columns: [
                                        ...
                                        {"data" : 'username'},
                                        {
                                            ...
                                            "render" : function(data, type, row){
                                                if(row.status_pengajuan_fungsional == 3){
                                                    return "<a href='<?= base_url('dashboard_p/ajuan_naikpangkat_fungsional/')?>"+data+"' class='btn btn-warning btn-sm'>Edit</a>";
                                                }
                                                else if(row.report_pengajuan_fungsional != null) {
                                                    return "<a href='<?= base_url()?>upload/report_naikpangkat/fungsional/"+row.report_pengajuan_fungsional+"' class='btn btn-success btn-sm' download>Download SK</a>";
                                                }
                                                else {
                                                    return '';
                                                };
