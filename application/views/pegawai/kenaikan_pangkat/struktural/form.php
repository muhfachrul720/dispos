<!-- <?php var_dump($aju); //die;?> -->
<?php if($id_kat_jbt_struktur != 0 || $id_kategori_jabatan_fung != 0) { ?>
    <?php if(notif_kenaikanpangkat($tmt_jab_struktural == null ? $tmt_jab_fungsional : $tmt_jab_struktural) != false) { ?>
            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane show active fade" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <div class="card border-top-0 mb-3 pt-3 px-3 pb-1">
                        <div class="card-body">
                            <h3 class="">Form Pengajuan Pangkat Jabatan Struktural </h3>
                            <p>Silahkan Mengupload syarat syarat yang diperlukan dalam mengurus Pensiunan Secara Lengkap dan benar</p>
                            <?php if(isset($aju)) {?> 
                                <div class="alert alert-info alert-sm" role="alert">
                               <small> Keterangan Koreksi : <?= $aju['keterangan_pengajuan_struktural']?> </small>
                            </div>
                            <?php };?> 
                        </div>
                    </div>  
                    ...
                    <?php $checker = array(
                        ...
                        array('<b>Foto Kopi Sah</b> SK Jabatan', 'srtjab'),
                        array('<b>Foto Kopi Sah</b> Surat Pernyataan Pelantikan', 'srtlatih'),
                        array('<b>Foto Kopi Sah</b> Surat Pernyataan Meduduki Jabatan', 'srtdukjab'),
                        array('<b>Foto Kopi Sah</b> Surat Perintah Melaksanakan Tugas', 'srttgs'),
                        array('<b>SKP, Capaian SKP, dan Penilaian Prestasi Kerja</b> (2 Tahun terakhir <b>sekurang-kurangnya bernilai baik</b>', 'skp'),
                    );?>
                    <?php if(isset($aju)){ 
                        echo form_hidden('time', $aju['waktu_pengajuan_struktural']);
                        echo form_hidden('id', $aju['id_ajuan_struktural']);
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-5">
                                            ...
                                            <?php echo cmb_dinamis('usuljab', 'tbl_kategori_jabatan_struktur', 'nama_jabatan_struktur', 'id_kat_jbt_struktur', isset($aju['usulan_jabatan_struktural']) ? $aju['usulan_jabatan_struktural'] : '', 'DESC') ?>
                                        </div>
                                    </div>
                                    <hr>
                        ...
                        <script type="text/javascript">
                            $(document).ready(function() {
                                ...
                                var t = $("#example").dataTable({
                                    ...
                                    processing: true,
                                    serverSide: true,
                                    ajax: {"url": '<?= base_url()?>dashboard_p/json_naikpangkat_struktural', "type": "POST", data : {'id' : <?= $this->session->userdata('id_pegawai')?>}},
                                    columns: [
                                        {"data" : 'waktu_pengajuan_struktural', orderable:false},
                                        {"data" : 'waktu_pengajuan_struktural'},
                                        {
                                            "data" : 'status_pengajuan_struktural',
                                        ...
                                        {"data" : 'keterangan_pengajuan_struktural'},
                                        {"data" : 'username'},
                                        {
                                            "data" : 'id_ajuan_struktural',
                                            "render" : function(data, type, row){
                                                if(row.status_pengajuan_struktural == 3){
                                                    return "<a href='<?= base_url('dashboard_p/ajuan_naikpangkat_struktural/')?>"+data+"' class='btn btn-warning btn-sm'>Edit</a>";
                                                }
                                                else if(row.report_pengajuan_struktural != null) {
                                                    return "<a href='<?= base_url()?>upload/report_naikpangkat/struktural/"+row.report_pengajuan_struktural+"' class='btn btn-success btn-sm' download>Download SK</a>";
                                                }
                                                else {
                                                    return '';
                                                };
    ...
    <?php } else { ?>
        <div class="container-fluid">
            <h2 style="text-align:center; margin-top:20vh">Anda Belum Dapat Mengajukan Kenaikan Pangkat Struktural <br> Silahkan Mengecek Apakah Anda Telah Memenuhi Kriteria 2 Tahun Masa Kerja Jabatan Tersebut</h2>
        </div>
    <?php };?>
<?php } else {?>
        <div class="container-fluid">
            <h2 style="text-align:center; margin-top:20vh">Hanya Pegawai Yang Memegang Jabatan Yang Dapat Mengajukan Kenaikan Pangkat</h2>
        </div>
<?php };?> 
